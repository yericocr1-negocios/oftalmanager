...
export default function Agenda() {
  ...
  const [nuevaCita, setNuevaCita] = useState({ paciente: '', doctor: 'Dr. Garcia', especialidad: '', hora: '', fecha: '', duracion: 30 })

  const citasFiltradas = filtroDoctor === 'todos' ? citasState : citasState.filter(c => c.doctor === filtroDoctor)

  const cambiarEstado = (id, nuevoEstado) => {
    setCitasState(citasState.map(c => c.id === id ? { ...c, estado: nuevoEstado } : c))
  }
        ...
        <div className="p-8">
          <div className="flex gap-3 mb-6">
            {['todos', 'Dr. Garcia', 'Dra. Torres'].map((doc) => (
              <button key={doc} onClick={() => setFiltroDoctor(doc)} className={'px-4 py-2 rounded-lg text-sm transition-all ' + (filtroDoctor === doc ? 'bg-blue-600 text-white' : 'bg-gray-900 text-gray-400 hover:bg-gray-800')}>
          ...
          <div className="grid grid-cols-4 gap-4 mb-8">
            {[
              { label: 'Total citas', value: citasState.length, color: 'text-white' },
              { label: 'Confirmadas', value: citasState.filter(c => c.estado === 'confirmada').length, color: 'text-green-400' },
              { label: 'En atencion', value: citasState.filter(c => c.estado === 'en_atencion').length, color: 'text-orange-400' },
              { label: 'No vinieron', value: citasState.filter(c => c.estado === 'no_vino').length, color: 'text-red-400' },
            ].map((card) => (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 w-full max-w-lg">
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={() => setMostrar(false)} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg text-sm font-medium">Guardar cita</button>
            </div>
          </div>
        </div>
      )}
